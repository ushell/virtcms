...
class  Virt {
    ...
    /**
     * create image
     * Notice: qemu-img command must exists
     * @param $name  absolute path [eg: /data/image/xx.qcow2]
     * @param $size [/Mib]
     * @param $baseimg [base image for instances]
     */
    function domain_create_disk($name, $size, $format, $baseimg=''){
        if(isset($baseimg) && !empty($baseimg)){
            $name = C('LIBVIRT')['increment_storage_path'].$name;
            $command = "qemu-img create -b {$baseimg} -f {$format} {$name}";
            exec($command, $out, $ret);
            return array('status'=>$ret, 'msg'=>$out[0]);
        } else {
            return libvirt_image_create($this->conn, $name, $size, $format);
        }
    }

    /**
     * remove image file
     * Notice: vmname is vmname's file name on local system
     * @param vmname [absolute path!]
     * @return bool
     */
    function domain_disk_delete($imageFile){
        $tmp = libvirt_image_remove($this->conn, $imageFile);
        return ($tmp) ? $tmp : $this->_set_last_error();
    }
    /**
     * image | dev[hda or sdb] | type[ide or scsi] | driver[raw or qcow2]
     * define XML ,next create disk
     */
    ...
    /**
     * create xml persistent($persistent=true)
     *
     */
    function domain_create_xml($xml, $persistent=true, $vmName=''){
        if($persistent){
            $tmp = libvirt_domain_define_xml($this->conn, $xml);
            if(!$tmp){
                return $this->_set_last_error();
            } else {
                //$tmp = $this->domain_start($vmName); //创建后不启动
                return ($tmp) ? $tmp : $this->_set_last_error();
            }
    ...
    /**
     * get system emualtor
     * return /usr/xx/qemu-kvm
     */
    function domain_get_emulator(){
        return libvirt_connect_get_emulator($this->conn);
    }
    ...
    function domain_state($domain){
        ...
        $vmState    =   $tmp['state'];
        $vmCpuState =   $tmp['cpuUsed']; //float

        $ret = array('state'=>$vmState, 'cpu'=>$vmCpuState);
        return $ret;
    }
    /**
     * $domain['state']
     * VIR_DOMAIN_NOSTATE	=	0 no state
     * VIR_DOMAIN_RUNNING	=	1 the domain is running [x]
     * VIR_DOMAIN_BLOCKED	=	2 the domain is blocked on resource
     * VIR_DOMAIN_PAUSED	=	3 the domain is paused by user [x]
     * VIR_DOMAIN_SHUTDOWN	=	4 the domain is being shut down
     * VIR_DOMAIN_SHUTOFF	=	5 the domain is shut off [x]
     * VIR_DOMAIN_CRASHED	=	6 the domain is crashed
     * VIR_DOMAIN_PMSUSPENDED=	7 the domain is suspended by guest power management
     * VIR_DOMAIN_LAST	    =	8  NB: this enum value will increase over time as new events are added to the libvirt API
     */
